...
int main () {
	...
	//resstring1 - строка полученная ребенком
	...
	if(result < 0) {
	       ...
	       exit(-1);
		      ...
		      printf("закрыть входной или выходной поток процесса родителя\n");
		      exit(-1);
	}
	     ...
	     printf("Читаем строку, отправленную ребенком ...\n");
		...
		if (close(fd2[0]) < 0) {
	     ...
	     printf("процесс родитель завершил работу\n");
          ...
          if ((close(fd1[1])<0)||(close (fd2[0]) <0)) {
                        printf("не удалось закрыть выходной и входной поток процесса ребенка\n");
		 exit(-1);
	  }
	  ...
	  printf("строка прочитанная дочерним процессом: %s\n", resstring1);
	    ...
	    if (close(fd2[1])<0) {
		    printf("Не удалось закрыть выходной поток проуеса-ребенка");
			    exit(-1);
	}
